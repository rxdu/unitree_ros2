...
#define HIGH_FREQ \
  1  // Set 1 to subscribe to low states with high frequencies (500Hz)
...
class low_level_cmd_sender : public rclcpp::Node {
 public:
  low_level_cmd_sender() : Node("low_level_cmd_sender") {
    ...
    lowstate_subscriber_ = this->create_subscription<unitree_hg::msg::LowState>(
        topic_name, 10,
        std::bind(&low_level_cmd_sender::LowStateHandler, this, _1));
    ...
    timer_ = this->create_wall_timer(
        std::chrono::milliseconds(timer_dt),
        std::bind(&low_level_cmd_sender::Control, this));
 ...
 private:
  ...
  unitree_hg::msg::MotorState
      motor[H1_2_NUM_MOTOR];   // Unitree hg motor state message
